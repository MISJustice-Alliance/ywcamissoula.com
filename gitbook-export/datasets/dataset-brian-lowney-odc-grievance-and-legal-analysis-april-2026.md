---
title: >-
  Dataset: Brian Lowney ODC grievance and legal analysis (April 2026)
description: >-
  Dataset landing page for the Brian Lowney ODC grievance filed April 28,
  2026. Includes the full grievance document, legal analysis of prosecutorial
  misconduct claims, and cross-references to the broader Montana bar
  complaints record.
---


# Dataset: Brian Lowney ODC grievance and legal analysis (April 2026)

### Executive snapshot

This dataset contains the grievance filed with the Montana Office of Disciplinary Counsel (ODC) against attorney Brian Lowney on April 28, 2026, together with legal analysis of its significance within the broader pattern of alleged prosecutorial misconduct and bar disciplinary failures in Missoula County.

**Current status:** ODC dismissed the grievance by letter dated June 9, 2026, citing MRLDE Rule 11(A)'s limitations period and stating that the "charges happened in 2019." A Commission on Practice review request has been prepared and added to this dataset to address ODC's Rule 11 analysis and the underlying prosecutor-duty issues.

{% include "../.gitbook/includes/analysis-page-disclaimer-seo-geo-standard.md" %}

### Verify first (primary artifacts)

* Full grievance document: [Grievance to Montana ODC — Brian Lowney (April 28, 2026)](../montana-bar-complaints/grievance-to-montana-odc-brian-lowney-4.28.2026.md)
* ODC dismissal letter: [ODC File No. 26-145 Rejection — Brian C. Lowney (June 9, 2026)](../montana-bar-complaints/odc-26-145-rejection-brian-lowney-june-9-2026.md)
* Commission review request: [Request for Commission on Practice Review — ODC File No. 26-145 (Brian C. Lowney)](../montana-bar-complaints/request-for-commission-on-practice-review-odc-26-145-brian-lowney.md)
* Grievance index and metadata: [ODC Lowney Grievance Index](../montana-bar-complaints/odc-lowney-grievance-index.md)
* Related bar complaint (pattern context): [ODC 25-147 (Bryan Tipp) — Montana Bar Complaint index](../montana-bar-complaints/odc-25-147-bryan-tipp-montana-bar-complaint-index.md)
* Source/citation hub: [Sources & record index](../overview/sources-and-record-index.md)
* Structured-data values: [Structured data registry (Dataset values)](structured-data-registry-dataset-values.md)

### Dataset scope

| Field | Value |
|-------|-------|
| **Grievance date** | April 28, 2026 |
| **Respondent** | Brian Lowney |
| **Filing jurisdiction** | Montana Office of Disciplinary Counsel (ODC) |
| **Case type** | Montana Bar grievance / prosecutorial misconduct |
| **Current posture** | Dismissed by ODC June 9, 2026; Commission on Practice review request prepared before July 9, 2026 deadline |
| **Related case files** | CR-2025-001, CR-2025-002, CR-2025-003 |
| **Pattern linkage** | Missoula County prosecutor misconduct; follows ODC 25-147 (Bryan Tipp) |

### Primary documents

1. **Grievance to Montana ODC — Brian Lowney (April 28, 2026)**
   * Full text of the grievance filing.
   * [Read the grievance](../montana-bar-complaints/grievance-to-montana-odc-brian-lowney-4.28.2026.md)

2. **ODC Lowney Grievance Index**
   * Structured metadata, claims summary, and navigation hub.
   * [View the index](../montana-bar-complaints/odc-lowney-grievance-index.md)

3. **ODC File No. 26-145 Rejection — Brian C. Lowney (June 9, 2026)**
   * Public record page for ODC's limitations-based dismissal letter.
   * [Read the rejection page](../montana-bar-complaints/odc-26-145-rejection-brian-lowney-june-9-2026.md)

4. **Request for Commission on Practice Review — ODC File No. 26-145**
   * Expanded review request with Montana and federal authority on Rule 11, prosecutor duties, Brady/Giglio, and DPA speech-waiver issues.
   * [Read the review request](../montana-bar-complaints/request-for-commission-on-practice-review-odc-26-145-brian-lowney.md)

### Legal analysis summary

The Lowney grievance advances allegations of prosecutorial misconduct that complement and extend the pattern documented in the ODC 25-147 complaint against Bryan Tipp. Key analytical significance:

* **Pattern reinforcement**: If the ODC acknowledges merit in the Lowney grievance, it corroborates the systemic-prosecutorial-misconduct theory advanced across CR-2025-001 through CR-2025-003.
* **Timeline continuity**: The April 2026 filing maintains the evidentiary chronology and demonstrates ongoing institutional accountability gaps.
* **Cross-reference density**: The grievance references the same underlying fact patterns (YWCA institutional capture, MPD evidentiary failures, and coercive plea dynamics) that anchor the broader Nuno case record.

### Structured data (Schema.org Dataset)

```json
{
  "@context": "https://schema.org",
  "@type": "Dataset",
  "name": "Brian Lowney ODC Grievance and Legal Analysis (April 2026)",
  "description": "Grievance filed with Montana ODC against attorney Brian Lowney, with legal analysis of prosecutorial misconduct claims and cross-references to broader Montana bar complaints.",
  "url": "https://www.ywcaofmissoula.com/datasets/dataset-brian-lowney-odc-grievance-and-legal-analysis-april-2026",
  "sameAs": [],
  "identifier": "ywcaofmissoula-brian-lowney-odc-grievance-2026",
  "isAccessibleForFree": true,
  "temporalCoverage": "2026-04-28/2026-04-28",
  "spatialCoverage": {
    "@type": "Place",
    "name": "Missoula County, Montana"
  },
  "isPartOf": {
    "@type": "Dataset",
    "name": "Nuno case civil-rights violations and timeline (2014–2025)"
  }
}
```

### Related

* [Dataset catalog (Google indexing)](dataset-catalog-indexes.md)
* [Sources & record index](../overview/sources-and-record-index.md)
* [Washington/Montana Legal Cases Index](../washington-montana-legal-cases-index.md)
* [Federal and State Department Complaints](../state-and-federal-complaints/federal-and-state-department-complaints.md)
